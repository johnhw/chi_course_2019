...
def box_probability(particles, weights, boxes):    
    ...
    for box in boxes:
        ...
        posterior = np.sum(weights[np.logical_and(xs>box.left, xs<box.right)] * box.prior)
...
class SliderDemo(object):
    ...
    def update_filter(self):
        ...
        self.particles, self.weights = slider_filter.filter_step(self.particles, observed=observation, dt=self.dt, prior_rate=0.00005)
    ...
    def draw(self, src):        
        ...
        for box, box_w in zip(self.boxes, box_weights):
            ...
            if self.show_particles:
                ...
                src.rectangle(screen_left, 0, screen_right, self.slider_height, fill=scaled_color, outline=box.color)
            else:
                src.rectangle(screen_left, 0, screen_right, self.slider_height, fill=box.color, outline=box.color)
        ...
        if self.show_particles:
            ...
            src.circle(expected_pos[0] * self.screen_size, self.slider_height//2, 5, fill='blue')
            src.line(expected_pos[0] * self.screen_size, 0, expected_pos[0] * self.screen_size,
            self.slider_height,
             fill='blue')

            src.line((expected_pos[0]-np.sqrt(expected_var[0])) * self.screen_size, self.slider_height//2, 
                    (expected_pos[0]+np.sqrt(expected_var[0])) * self.screen_size, self.slider_height//2, 
            fill='blue')

        if self.observed:
            src.line(screen_x, 0, screen_x, self.slider_height, fill="white", width=5)
            src.line(screen_x+5, 0, screen_x+5, self.slider_height, fill="black", width=5)
            
import argparse

def create_slider():
    boxes = [Box("A", 0.0, 0.1, prior=0.25, color='#ff7200'), 
