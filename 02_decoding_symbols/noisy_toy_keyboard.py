...
class NoisyToyKeyboard(object):
    ...
    def on_click(self,event):
        touchNoise = np.random.normal(0, self.sigma, 2)        
        xNoisy = event.x + touchNoise[0]
        yNoisy = event.y + touchNoise[1]
        
        # Animate the closest key and touch point
        self.animate_key(xNoisy,yNoisy)
        self.animate_touch(xNoisy,yNoisy)
        
        # Normalize touch point and log to active log file
        xTouch = (xNoisy - 150.0) / 100.0
        yTouch = (yNoisy - 150.0) / 100.0
        
        # Take random value to model insertions and deletions occuring 5% of the time each
        random_val = np.random.normal(0, 1, 1)
        
        # If below -0.165 add a spurious touch
        if (random_val < -1.65):
            print("spurious touch event")
            spuriousNoise = np.random.normal(0, self.sigma, 2)        
            xSpurious = ((event.x + touchNoise[0]) - 150.0 ) / 100.0
            ySpurious = ((event.y + touchNoise[1]) - 150.0 ) / 100.0
            self.touch_points = np.append(self.touch_points, np.array([[xSpurious,ySpurious]]), axis=0)

        # If above 0.165 don't append the touch point            
        if (random_val > 1.65):
            print("touch failed to be recorded event")        
        else:
    ...
    def create_new_log(self):
        ...
        filename = "stimulus_noisy_response.log"
    ...
    def __init__(self, master=None, sigma = 10):
        ...
        self.root.title("Noisy ABCD Keyboard")
